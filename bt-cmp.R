...
### where xmat and covariate must be matrices.
### Number of categories can't be too large for setparts().
split.node=function(xmat,node.cur,covariate,y,crit.parent,var.names,crit.min=0,maxsplit=8,minsize=20,reg.method="linear",ncate.low=10,ncate.high=40,F_test=F,ncate.cont=10)
{
...
################################################################################ Fitting a simple tree with M nodes based on OLS.############################################################################################

Simple.tree=function(x.mat,nodecur="1",covariate,y,crit.root,var.names,M=4,crit.min=0,maxsplit=8,minisize=20,regmethod="linear",ncate_low=20,ncate_high=40,F.test=F,L.cont=10)
{
  leaves.cur=nodecur
  split.rule=c()
...
############################################################
############## predict function for each split ############
#########################################################
######
...
split.predict=function(xmat,covariate,split.rule,leaf.summary,cols=3:(ncol(leaf.summary)-1))
{
  #cat(split.rule,'\n')
  ...
  for(i in 1:nrow(leaf.summary))
  {
    ...
    if(length(leaf.pred[logic.v]) != nrow(as.matrix(hehe))) cat(length(leaf.pred[logic.v]),nrow(as.matrix(hehe)) ,"Weird!",'\n')
...
################################################################################ main boosting function for cmp #################################
#########################################################################

Boost.Tree=function(formula,fformula=NULL,vcformula=NULL, data,M=4,nBoost=500,eta,mini.size=20,ncatelow=20,ncatehigh=40)
{
  #############################################################
  ### Data preparation and global model fitting #########
  ################################################################
    # for main formula
    ...
    if(!is.null(tvc2)) Z1 <- model.frame(tvc2,data = data) else Z1=0
    n <- nrow(Z)
    
    
    ## Global Model
    mod <- gam.fit.cmp(G,G1,family=cmp(),control=gam.control())
    # G$mucoef <- mod$coefficients
    # G1$nucoef <- mod$coefficients.nu
    if(dim(G$X)[2]>nvcvar) fixEf <- as.matrix(G$X[,-c(1:nvcvar)])%*%mod$coefficients[-c(1:nvcvar)]
    else fixEf <- rep(0,G$n)
    if(dim(G1$X)[2]>nvcvar1) fixEf.nu <- as.matrix(G1$X[,-c(1:nvcvar1)])%*%as.vector(mod$coefficients.nu[-c(1:nvcvar1)])
    else fixEf.nu <- rep(0,G1$n)
  
    ##################################################################
  ### Preparation for Boosting #######################################
    #################################################################
  coef.Boost=matrix(rep(mod$coefficients[c(1:nvcvar)],G$n),G$n,nvcvar,byrow = TRUE)
  coef.Boost.nu=matrix(rep(mod$coefficients.nu[c(1:nvcvar1)],G1$n),G1$n,nvcvar1,byrow = TRUE)
  
  # coef.Boost[,1]=mean(varY)
  varReg1 <- G$X[,c(1:nvcvar)]
  varReg2 <- as.matrix(G1$X[,c(1:nvcvar1)]) 

  cum1=CMPCumulants(llambda = apply(varReg1*coef.Boost,1,sum)+fixEf,lnu = apply(varReg2*coef.Boost.nu,1,sum)+fixEf.nu,flag = 1)
  resid.cur1=(G$y-cum1$mean)/sqrt(cum1$var)
  cum2=CMPCumulants(llambda = apply(varReg1*coef.Boost,1,sum)+fixEf,lnu = apply(varReg2*coef.Boost.nu,1,sum)+fixEf.nu,flag = 2)
  ylogfact.vec=sapply(G$y, LogFactorial)
  resid.cur2=(cum2$mean-ylogfact.vec)/sqrt(cum2$var)
  
  crit.root=mod$deviance
  #cat(mean(varY))
  #print(coef.Boost)
  var.imp1=rep(0,dim(Z)[2])
  if(!is.null(tvc2)) var.imp2=rep(0,dim(Z)[2]) else var.imp2=0
  #
  splt.crit1=c();splt.crit2=c() 
  splt.modelest1=c(); splt.modelest2=c()
  
  nlNu <- rep(0,n);nlLambda <- rep(0,n)
  dev.cur=rep(0,nBoost)
  err.sq <- dev.cur
 
  nu.temp <- exp(apply(varReg2*coef.Boost.nu,1,sum)+fixEf.nu)
  nlLambda <- apply(varReg1*coef.Boost,1,sum)+fixEf
  nlNu <- apply(varReg2*coef.Boost.nu,1,sum)+fixEf.nu
  #
  eta.vec <- rep(0,nBoost)
  ##
  for(iBoost in 1:nBoost)
  {
    # if(iBoost>100) eta=0.1
    # if(iBoost>200) eta=0.05
    eta.vec[iBoost] <- eta
    #
    cum1=CMPCumulants(llambda = nlLambda,lnu = nlNu,flag = 1)
    resid.cur1=(G$y-cum1$mean)/sqrt(cum1$var)
    tvarReg1 <- diag(sqrt(cum1$var))%*%varReg1
        ## Tree improvement for lambda model
    res1=Simple.tree(Z,"1",tvarReg1, resid.cur1,crit.root,var.names = names(Z),M,crit.min=0,minisize=mini.size,ncate_low=ncatelow,ncate_high=ncatehigh)
    res1.pred=split.estimate(res1$leaf.assoc,varReg1,res1$model.est,3:(nvcvar+2))
    res1.pred$coefs[is.na(res1.pred$coefs)] <- 0
    coef.increment1=eta*res1.pred$coefs
    var.imp1=var.imp1+(eta^2)*res1$split.impvec
    coef.Boost=coef.Boost+coef.increment1
    #
    
    varReg3 <- diag(drop(nu.temp))%*%varReg2
    ...
    nlLambda <- apply(varReg1*coef.Boost,1,sum)+fixEf
    #
    cum2=CMPCumulants(llambda = nlLambda,lnu = nlNu,flag = 2)
    resid.cur2=(cum2$mean-ylogfact.vec)/sqrt(cum2$var)
    tvarReg3 <- diag(sqrt(cum2$var))%*%varReg3
    #
    if(!is.null(tvc2))
    {
      res2=Simple.tree(Z1,"1",tvarReg3, resid.cur2,crit.root,var.names = names(Z1),M,crit.min=0,minisize=mini.size,ncate_low=ncatelow,ncate_high=ncatehigh)
      res2.pred=split.estimate(res2$leaf.assoc,varReg2,res2$model.est,3:(nvcvar1+2))
      coef.increment2=eta*res2.pred$coefs
      var.imp2=var.imp2+(eta^2)*res2$split.impvec
      if(sum(res2.pred$coefs)==0) coef.Boost.nu=coef.Boost.nu+eta*sum(sqrt(cum2$var)*resid.cur2)/sum(cum2$var) else coef.Boost.nu=coef.Boost.nu+coef.increment2
      splt.modelest2[[iBoost]]=res2$model.est
      splt.crit2=c(splt.crit2,res2$split.krit) 
    }else coef.Boost.nu=coef.Boost.nu+eta*lm.fit(tvarReg3,resid.cur2)$coefficients
    
    
    #sum(sqrt(cum2$var)*resid.cur2)/sum(cum2$var)
      
      
      # weights = sum wiyi/sum wi; but the resid.cur2 already has one wi^{1/2} multiplied.. sum(sqrt(cum2$var)*resid.cur2)/sum(sqrt(cum2$var)) 
    ...
    # 
    # cum1=CMPCumulants(llambda = nlLambda,lnu = nlNu,flag = 1)
    # resid.cur1=(G$y-cum1$mean)/cum1$var
    # 
    # 
    # cum2=CMPCumulants(llambda = nlLambda,lnu = nlNu,flag = 2)
    # resid.cur2=(cum2$mean-ylogfact.vec)/cum2$var
    # 
    # ylogfact.vec=sapply(varY, LogFactorial)
    
    dev.cur[iBoost]=CMP.dev(G$y,ylogfact.vec,lmu =nlLambda ,lnu=nlNu)
        #
    err.sq[iBoost] <- sum((G$y-cum1$mean)^2)
    crit.root=dev.cur[iBoost]
    splt.modelest1[[iBoost]]=res1$model.est
    splt.crit1=c(splt.crit1,res1$split.krit)
    
    ### Printing for convergence check
    cat(iBoost,":",dev.cur[iBoost],'\n')
    cat(iBoost,":",err.sq[iBoost],'\n')
    if(iBoost>20)
      {
      if(abs((dev.cur[iBoost-1]-dev.cur[iBoost])/(0.1+dev.cur[iBoost]))<=1e-5)  break  else if((dev.cur[iBoost]-dev.cur[iBoost-1])/(dev.cur[iBoost-1])>1e-3 &  iBoost > 50) break
      }
## global for loop    
  }
  fitted <-cum1$mean
    
  f0.hat.lam <- G$X[,c(1:nvcvar)]%*%as.matrix(mod$coefficients[c(1:nvcvar)])+fixEf
  f0.hat.nu <- G1$X[,c(1:nvcvar1)]%*%as.matrix(mod$coefficients.nu[c(1:nvcvar1)])+fixEf.nu
  
  #R2.in=unlist(R2.in)
  #colnames(error.in)=c("L2","L1")
  return(list(variable.imp.lam=var.imp1,variable.imp.nu=var.imp2,model.est.lam=splt.modelest1,model.est.nu=splt.modelest2,split.krit.lam=splt.crit1,split.krit.nu=splt.crit2,coefs.lam=coef.Boost,coefs.nu=coef.Boost.nu,base=mod,deviance=dev.cur,f0.hat.lam=f0.hat.lam,f0.hat.nu=f0.hat.nu,iter=iBoost,fitted.val=fitted,varPart1=Z,varPart2=Z1,rform=c(cf1,cf2),vform=c(tvc1,tvc2),data=data,M=M,nvc.lam=nvcvar,nvc.nu=nvcvar1,G=G,G1=G1,eta.vec=eta.vec,eta=eta,error.sq=err.sq,boost.lam=nlLambda,boost.nu=nlNu)) #,R2in=R2.in,f0.hat=mean(varY),error.train=error.in)
}
################################################################################
...
#   for(iBoost in 1:nBoost)
#   {
#     #xmat,covariate,split.rule,leaf.summary
#     spt.pred=split.predict(xmat=varPart,covariate=varReg,split.rule=obj.Boost$split.krit.lam[iBoost],leaf.summary=obj.Boost$model.est.lam[[iBoost]])
...
#######################################################################
######## Prediction for partial dependecy plots ######################
######################################################################

partial.pred <- function(varPart,obj,lnu.flag=1)
{
  pred=rep(0, obj$G$n)
  if(lnu.flag==1) 
    {
    varReg <- obj$G$X[,c(1:obj$nvc.lam)]
    split.krit <- obj$split.krit.lam
    leaf.sum <- obj$model.est.lam
    }else 
      {
        varReg <- obj$G1$X[,c(1:obj$nvc.nu)]
        split.krit <- obj$split.krit.nu
        leaf.sum <- obj$model.est.nu
      }
  #
  coef.mat <- varReg-varReg
  nBoost <- obj$iter
  ...
  #
  if(!is.null(split.krit))
  {
  for(iBoost in 1:nBoost)
  {
    # arguments: xmat,covariate,split.rule,leaf.summary
    # for general
    spt.pred=split.predict(xmat=varPart,covariate=varReg,split.rule=split.krit[iBoost],leaf.summary=leaf.sum[[iBoost]])
    pred=pred+spt.pred$pred*eta
    coef.mat=coef.mat+spt.pred$coefs*eta
  }
  }
  # retun
  return(list(pred=pred,coefs=coef.mat))
}
...
Partial.Boost=function(obj,name.vars)
{
  ##
  nn <- length(name.vars)
  n <- obj$G$n
  ...
  varPart.lam <- obj$varPart1 
  varPart.nu <- obj$varPart2
  varReg.lam <- obj$G$X[,c(1:obj$nvc.lam)]
  varReg.nu <- obj$G1$X[,c(1:obj$nvc.nu)]
  ...
  for(j in 1:nn)
  {
    ...
    if (is.null(var.levels.nu)& is.null(var.levels.lam)) stop("variable not found")
    ...
    coef.nu <- matrix(0,length(var.levels.nu),obj$nvc.nu)
    #
    if(!is.null(var.levels.lam))
    {
      ...
      if (is.null(var.levels.nu)){
        fits.nu[,j] <- apply(as.matrix(varReg.nu)*obj$coefs.nu,1,sum)
        ufits.nu[,j] <- fits.nu[order(varPart.lam[,name.var]),j] 
      }else ufits.nu[,j] <- 0
      ufits.lam[,j] <- fits.lam[order(varPart.lam[,name.var]),j]
    }
    if(!is.null(var.levels.nu) && obj$nvc.nu>1)
    {
      uniq.levels.nu[1:length(var.levels.nu),j] <- var.levels.nu
      for(ilevels.no in 1:length(var.levels.nu))
      {
        ilevels=var.levels.nu[ilevels.no]
        varPart.nu.cur[,name.var]=ilevels
        # cat(ilevels,': for nu \n')
        # varPart,obj,lnu.flag=1
  ...
  return(list(coefs.lam=coefs.lam,coefs.nu=coefs.nu,fits.lam=fits.lam,fits.nu=fits.nu,ulevel.lam=uniq.levels.lam,ulevel.nu=uniq.levels.nu,ufits.lam=ufits.lam,ufits.nu=ufits.nu))
}
...
#     cat(ilevels,'\n')
#     pred.cur=pred.Boost(varPart.cur,varReg,varY,res,3:4,nBoost.param,nu=nu.param)
...
regression=function(y,x,N.vec=0,n.vec=0,market=NA,method="linear",nEM=5000,tolern=1e-6,opt='exp',nests=NA,off.set=0,param.init=0)
{
  ...
  if(method=="nest")
  {
    ...
    crit.nest=function(param)
    {
      ...
      tau=rep(as.vector(c(param[-(1:(ncol(x)))])),length(unique(as.character(nests))))
      #tau[length(tau)]=0
      #tau=c(tau,0)
...
########################################################################
########### Boost cmp tree with the same split #########################
########################################################################

################################################################################ main boosting function for cmp #################################
#########################################################################

Boost.Tree.basic=function(formula,fformula=NULL,vcformula=NULL, data,M=4,nBoost=500,eta,mini.size=20,ncatelow=5,ncatehigh=20)
{
  #############################################################
  ### Data preparation and global model fitting #########
  ################################################################
