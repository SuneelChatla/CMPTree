...
Z <- model.frame(tvc1,data = data)
  ...
  if(dim(G$X)[2]>nvcvar) G$fixEf <- as.matrix(G$X[,-c(1:nvcvar)])%*%G$mucoef[-c(1:nvcvar)]
  else G$fixEf <- rep(0,G$n)
  if(dim(G1$X)[2]>nvcvar1) G1$fixEf <- as.matrix(G1$X[,-c(1:nvcvar1)])%*%as.vector(G1$nucoef[-c(1:nvcvar1)])
  else G1$fixEf <- rep(0,G1$n)
  ...
  if(dim(G$X)[2]>nvcvar) G$fixEf.up <- as.matrix(G$X[,-c(1:nvcvar)])%*%as.vector(G$mucoef.up)
  else G$fixEf.up <- rep(0,G$n)
  if(dim(G1$X)[2]>nvcvar1) G1$fixEf.up <- as.matrix(G1$X[,-c(1:nvcvar1)])%*%as.vector(G1$nucoef.up)
  else G1$fixEf.up <- rep(0,G1$n)
  ...
  return(list(tree=rval,fixed=nmod,glob=mod,fitted.values.y = fitted.values.y,coef.mob.lam=coef.mob.lam,coef.mob.nu=coef.mob.nu,Z=Z,nvc.lam=nvcvar,nvc.nu=nvcvar1))
}
    ...
    if(is.numeric(zselect)) {
      ...
      ## if starting values are not reused then the applyfun() is used for determining the split
          ...
          if(w2n(weights[zs]) < minsize || w2n(weights[!zs]) < minsize) {
            ...
            if(inherits(fit_left, "try-error")|inherits(fit_right, "try-error")) return(Inf)
            else return(fit_left$deviance + fit_right$deviance)
          }
        }
